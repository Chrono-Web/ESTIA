...
Le proprietà che lo rendono vero sono già scelte, ed è utile vederle in fila: la federazione come confine e non come schema ([ADR 0002](adr/0002-activitypub-confine-non-schema.md)), il primo contatto senza autorità esterne ([ADR 0003](adr/0003-primo-contatto-in-rete-locale.md)), le istanze che si cercano per chiave e si parlano direttamente ([ADR 0018](adr/0018-federazione-fra-istanze-estia.md)), i backup riapribili senza ESTIA ([ADR 0013](adr/0013-backup-cifrati-in-formato-age.md)), l'export come diritto (§8), zero server dello sviluppatore.

Una di queste ha una condizione che va detta e non sottintesa: quando due router non si lasciano attraversare serve un **relay** a fare da ponte, e se quel relay fosse uno solo, di un'azienda, la frase qui sopra sarebbe falsa. Vale quindi solo con **scoperta su una DHT senza proprietario e relay molti, ospitabili da chiunque** — che è ciò che ADR 0018 adotta, e non un dettaglio di implementazione.
...
Nella rete ESTIA la forma di questa tensione è precisa, perché i contenuti non si replicano ([ADR 0018](adr/0018-federazione-fra-istanze-estia.md)): **chi ti legge bussa a casa tua**, quindi la tua macchina sa quando succede, e **a macchina spenta non sei leggibile**. Su ActivityPub invece le copie si fanno e restano dove sono arrivate — è la ragione per cui quella porta si apre per scelta, e non è aperta per tutti.
