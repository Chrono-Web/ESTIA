...
**L'istanza ha già la chiave giusta.** Dal primo avvio genera una coppia **ed25519** ([`identity.ts`](../../apps/core-api/src/instance/identity.ts)), che è la stessa forma di identità che iroh usa per indirizzare. Se riusarla o derivarne una separata per il trasporto è una domanda di igiene crittografica da sciogliere in implementazione, e la prudenza dice di derivarla.
...
**Requisito che questo ADR fissa:** un'istanza deve poter federare **senza dipendere da infrastruttura di n0**. Le strade documentate esistono — DHT Mainline al posto del DNS, un `iroh-dns-server` proprio, un relay proprio — e quale adottare come default si decide misurando. Ciò che non è ammesso è ereditare il default senza accorgersene.
...
Nessuna riga di codice prima di queste, in quest'ordine:

1. **iroh su un NAS ARM vero.** Il pacchetto Node dentro l'immagine ESTIA, su `linux/arm64`, e due istanze che si collegano. Scioglie la deroga sui moduli nativi, e va fatto sull'hardware: questo progetto ha già scoperto due volte che le misure da qui mentono.
   - **Metà fatta il 2026-08-20, su `linux/amd64`.** `@number0/iroh` 1.1.0 installato dentro `node:24.18.0-bookworm-slim`, che è **l'immagine di base di ESTIA**: il modulo nativo si carica, e **due container separati si sono collegati per chiave pubblica** con il preset minimo — nessun relay, nessuna scoperta, nessuna infrastruttura di terzi — scambiandosi una richiesta e una risposta su uno stream QUIC bidirezionale. Il modulo nativo, che era la preoccupazione principale, non è un problema su questa architettura.
   - **Che cosa quella prova non dice**, e va tenuto separato: le due istanze erano sulla stessa macchina, quindi **l'attraversamento del NAT non è stato esercitato** — nessun hole punching, nessun relay. La parte che conta davvero, due case dietro due router, richiede due reti vere ed è la stessa classe di prova che tiene aperto il gate di M3.
   ...
   - **Dal 2026-08-20 la misura vive dentro l'istanza**, invece che in uno script a parte: `ESTIA_NETWORK_PROBE=local|internet` accende una sonda che pubblica l'identità di rete dell'istanza nel pannello, accetta un collegamento e ne risponde uno — un numero casuale rimandato indietro, **nessun contenuto**. Dal pannello si incolla il codice dell'altra istanza e si vede se è raggiungibile, in quanto tempo e **per collegamento diretto o via relay**, che è il dato che serve.
     ...
     - Da qui in avanti la verifica non richiede più software: richiede **due case**. È lo stesso genere di prova che tiene aperto il gate di M3, e il proprietario ha già due istanze su due linee diverse.
2. **Il costo di un profilo molto seguito.** Quante letture al secondo regge un NAS prima che il modello a visita diventi insostenibile, e a partire da quale soglia servirebbe una cache condivisa — che sarebbe una copia, e quindi una decisione da riaprire, non da improvvisare.
3. **Scoperta dei nodi e relay senza n0.** Provare la via alternativa e misurarne il costo, prima di scegliere il default.
4. **Il capitolo di sicurezza** qui sopra, scritto e discusso, prima di aprire una porta a istanze sconosciute.

Restano aperte e non bloccano: la forma del protocollo, il versionamento, il meccanismo dell'avviso vuoto, e il rapporto fra la chiave dell'istanza e quella del trasporto.

## Quando riesaminare

- Se la verifica 1 fallisce, il candidato cade e la decisione di modello resta, cercando un altro trasporto.
