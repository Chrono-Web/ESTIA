...
| **Collegata**   | Un amministratore di qua ha accettato, **oppure** un membro di qua segue qualcuno di là ([ADR 0018](0018-federazione-fra-istanze-estia.md)) | I profili **presenti** e i contenuti il cui scope include chi chiede; inoltrare una ricerca, a cui si risponde **solo per i profili pubblici propri** |
...
**Niente è enumerabile.** Non si può chiedere «dammi i tuoi membri», «dammi i tuoi post», «dammi le tue connessioni». Si chiede **qualcosa che si sa già nominare**. La differenza è tutta la differenza: l'enumerazione è il modo in cui un'istanza ostile raccoglie un grafo sociale, e il costo di vietarla è che la scoperta deve passare dalle relazioni — che è esattamente ciò che ADR 0018 aveva già deciso per altre ragioni. Le due decisioni si sostengono.
