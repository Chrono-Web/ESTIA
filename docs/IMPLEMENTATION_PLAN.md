...
- [x] **L'istanza si accorge se i propri dati non sopravvivranno all'aggiornamento**, aggiunto il 2026-08-17 dopo una perdita reale. Un'istanza installata dal pannello di un NAS senza mappare una cartella funziona perfettamente e **si azzera al primo aggiornamento**: il container viene buttato e rifatto, e con lui spariscono account, contenuti, fotografie e la chiave privata dell'istanza, che [`SECURITY_BASELINE.md`](SECURITY_BASELINE.md) §3 classifica come non sostituibile.
  ...
  - **Dichiarare `VOLUME /data` nell'immagine non risolve** — provato lo stesso giorno: `docker run` crea un volume anonimo nuovo a ogni ricreazione e i dati si perdono uguale. Avrebbe dato una falsa sicurezza, che è peggio del problema.
  - La strada è quella di [ADR 0007](adr/0007-cifratura-a-riposo-e-furto-fisico.md): **l'istanza guarda la propria tabella dei mount e lo dice.** Nei log come errore, nella diagnostica dell'amministratore in rosso, e — questa è la parte che conta — **nella schermata di configurazione**, prima che qualcuno ci metta dentro le fotografie di un quartiere. Mentre l'istanza è `unconfigured` l'unica persona che guarda è chi la sta installando.
  - Provato in container veri, con e senza volume, non solo nei test.
