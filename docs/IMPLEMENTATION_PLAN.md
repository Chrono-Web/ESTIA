...
1. **Profilo, con follow e richieste di follow.** **Metà costruita il 2026-08-20**: il profilo esiste, ha una presenza scelta dalla persona, ed è servibile in rete. Follow e richieste di follow **no** — vedi in fondo alla voce. Pagina profilo, profilo chiuso o aperto, e lo scope `followers` — che è nello schema dalla prima migrazione del feed e non è mai stato implementato. Comprende la scelta della cerchia al momento di pubblicare, che oggi l'interfaccia non offre di proposito: un post «pubblico» non raggiungerebbe nessuno finché non c'è federazione, e un'etichetta che mente è peggio di una funzione che manca.
   ...
   - **Costruito il 2026-08-20**: migrazione 10, tre presenze con `non_presente` come default nello schema oltre che nel dominio, i messaggi `profilo` e `cerca` sul protocollo, e la schermata del profilo con le conseguenze scritte accanto a ogni scelta invece di tre etichette.
     ...
     - **Manca il follow**, che è l'altra metà di questa voce: richieste, accettazione, e lo scope `followers` che è nello schema dalla prima migrazione del feed e continua a non essere implementato. Diviso qui perché il follow è una relazione **fra persone su istanze diverse**, che vuole il suo giro di decisioni — chi la conserva, che cosa succede quando l'istanza dell'altro è spenta, e come si revoca — e perché senza profili non aveva su cosa poggiare.
