...
Dopo [ADR 0003](adr/0003-primo-contatto-in-rete-locale.md) e [ADR 0004](adr/0004-client-web-e-trasporto-sostituibile.md) i confini erano cinque, e molto più semplici del piano originario: **non c'è esposizione pubblica, non c'è autorità di certificazione, non c'è control plane di terzi.** Vale per il prodotto come si installa e si usa: chi aggiunge il trasporto del pilot per leggere da fuori casa aggiunge con esso un terzo, ed è il confine 4 — dichiarato in [`ACCESSO_DA_FUORI.md`](ACCESSO_DA_FUORI.md) §5, dove si dice che cosa vede.
...
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Attaccante da Internet                      | Nessuna porta esposta e nessun indirizzo pubblico: da un indirizzo non si arriva qui                                                                                                                                                                                          | Coperto per costruzione                                                                                                       |
| Istanza remota che conosce la chiave        | Dal 2026-08-20 la rete fra istanze rende l'istanza raggiungibile **da chi possiede la sua chiave pubblica**, per scelta e senza aprire porte. Una sconosciuta può solo presentarsi; tutto il resto richiede un rapporto accettato, e il feed locale non è servibile a nessuno | Coperto da [ADR 0020](adr/0020-che-cosa-puo-chiedere-un-istanza-che-non-conosciamo.md), **da implementare** con il protocollo |
| Istanza remota che mente su chi ospita      | Nessuna difesa tecnica, e non ce ne sono: per entrare in un indice serve che qualcuno segua qualcuno, e un'istanza che si comporta male si blocca. L'interfaccia non presenta come verificato ciò che è dichiarato                                                            | **Scoperto, e dichiarato**                                                                                                    |
...
| Furto fisico del NAS                        | Cifratura del volume eseguita dal sistema ospite, passphrase all'avvio proposta come default ([ADR 0007](adr/0007-cifratura-a-riposo-e-furto-fisico.md)); l'istanza verifica e dichiara lo stato reale                                                                        | Coperto se l'amministratore cifra il volume — e l'istanza dice la verità in entrambi i casi                                   |
...
Restano scoperte tre righe, e sono diverse fra loro.
...
**Un'istanza remota che mente non si smaschera con la crittografia.** Può dichiarare profili che non ospita, e nessuna firma lo impedisce: la firma prova chi parla, non che dica il vero. La difesa è strutturale — si entra in un indice solo se qualcuno segue qualcuno, e chi si comporta male si blocca — e la conseguenza per il prodotto è che **il nome di un'istanza remota va mostrato come una cosa che lei dice di sé**, mai come un dato verificato.

Queste tre sono anche l'esatta misura di ciò che l'interfaccia **non può promettere**.
...
**Attuato il 2026-08-16.** L'istanza legge la pila dei dispositivi sotto la propria directory dei dati e riconosce uno strato `dm-crypt` anche quando sta sotto LVM; su ZFS e dove non può ispezionare risponde «non verificabile». Ma vede **se** un volume è cifrato, non **come viene sbloccato**: quella parte la dichiara chi amministra, e l'istanza confronta le due cose. **Se la dichiarazione afferma una cifratura che non risulta, l'interfaccia contraddice la dichiarazione** e i log registrano `at_rest_mismatch` — perché una protezione creduta e non presente è peggio di una protezione assente e nota.
...
- **Che cosa la cifratura end-to-end non protegge**: non è più un rinvio — M6 l'ha costruita, ed è `ESTIA-E2E-v1` ([ADR 0036](adr/0036-estia-e2e-v1-e-il-debito-verso-mls.md), che sostituisce ADR 0027: MLS era dichiarato e non è mai stato scritto). Copre il contenuto dei DM davanti a chi ospita, verificato da un test sul database e sui backup. **Restano scoperte quattro cose, e vanno lette come parte di questa baseline**: non c'è forward secrecy (una chiave compromessa apre tutto il passato e tutto il futuro di quella conversazione); l'uscita dell'ECDH diventa chiave AES senza KDF; la chiave non è legata alla conversazione; e **non esiste verifica fuori banda delle chiavi** — è l'istanza a distribuire i KeyPackage, quindi un'istanza compromessa può sostituirne una e leggere da lì in avanti. La verifica delle chiavi e la rotazione restano da progettare.
...
- **Rete fra istanze**: non è più un rinvio. Il capitolo è scritto — [ADR 0020](adr/0020-che-cosa-puo-chiedere-un-istanza-che-non-conosciamo.md), il 2026-08-20 — e ha già cambiato due cose qui dentro: il §1 ha la riga dell'istanza remota, e il §5 non dice più «non raggiungibile da Internet» come se fosse ancora vero. Quel documento resta **da implementare con il protocollo**: finché il protocollo non esiste, l'unica cosa che attraversa quel confine è la sonda di ADR 0018, che rimanda indietro un numero casuale e non tocca nessun contenuto.
