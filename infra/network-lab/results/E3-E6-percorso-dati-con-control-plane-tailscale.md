...
- Data: 2026-08-13
- Eseguito da: Palu, sulla propria linea domestica, con assistenza
...
| rDNS / whois     | `host-<ip>.business.operatore.example` · `<pool-dell-operatore>`, `<rete-dell-operatore>` |
| Router           | router dell'operatore — **amministrazione non accessibile** (vedi §Limiti)         |
| LAN              | `una /24 privata`; NAS `<nas>` su `.4`, Mac su `.105`                         |
| NAS              | `<nas>`, Linux, raggiungibile via tailnet                                    |
| Client mobile    | iPhone, iOS, client Tailscale ufficiale                                      |
| Operatore mobile | operatore mobile (instrada via `rete-di-transito.example`, rete di transito in Francia)               |
...
ping -c 20 -i 0.5 <ip-tailscale-iphone>    # campione a caldo
# ... iPhone bloccato, un minuto senza toccarlo ...
tailscale status | grep <telefono>        # osservazione passiva
ping -c 15 -i 0.5 <ip-tailscale-iphone>    # primo contatto a freddo
...
| DERP più vicino         | Francoforte, 54 ms | Costo del relay se il percorso diretto fallisse       |

**CGNAT: molto probabilmente assente.** L'IPv4 pubblico non è in `100.64.0.0/10`, il reverse DNS è generato uno-a-uno dall'indirizzo su dominio `business.operatore.example`, e il blocco è un pool DHCP di assegnazione diretta. Non è prova formale — manca la pagina WAN del router — ma è coerente con tutte le altre misure.
...
1. **Il collegamento diretto tra rete mobile e rete di casa funziona.** L'iPhone in rete mobile ha stabilito un percorso diretto verso la LAN dietro il router domestico, senza relay: `<endpoint-pubblico>:<porta>`, 100 ms. È il risultato più importante della serata — significa che ESTIA non ha bisogno di un relay obbligatorio su questa coppia di reti, e che i contenuti non transitano da infrastruttura di terzi.
...
4. **Ma il risveglio ha un costo misurabile.** A freddo la media raddoppia (277 ms contro 151) e il primo pacchetto arriva a **1,28 secondi**. Inoltre la porta dell'endpoint è cambiata durante l'inattività (`<porta>` → `<porta-dopo-il-risveglio>`): il percorso diretto viene rinegoziato, non semplicemente ripreso.

5. **La latenza di base dipende dall'operatore, non dall'architettura.** operatore mobile instrada il traffico mobile attraverso la rete di transito in Francia. I 150 ms medi sono routing fisico: nessuna scelta di control plane, protocollo o topologia li migliora.
...
- **Una sola coppia di reti e un solo operatore.** operatore mobile via Francia è un caso particolare, non una media.
