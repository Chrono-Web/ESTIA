...
- **Un membro con più di un dispositivo**, aperta il 2026-08-27: [ADR 0040](docs/adr/0040-un-membro-ha-piu-di-un-dispositivo.md), **Proposed**. Oggi ESTIA è **a un dispositivo per persona** e non lo dice: chi scrive riceve una sola chiave, la più recente, quindi aprire ESTIA dal telefono spegne il computer. MLS lo risolve — un dispositivo è una foglia — ma **non decide chi può aggiungere una foglia a nome tuo**, e chi non decide sceglie che basti la password dell'account. Va sciolta **prima di M8**: gruppi e foglie multiple toccano lo stesso meccanismo.
